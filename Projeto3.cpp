...
#include <EEPROM.h >  //Header EEPROM
...
//variaveis EEPROM
int enderecoEscrita = 0;//endereco para escrever novos dados, 16bytes little endian (menos significativo no endereco A, mais significativo no endereco A+1)
...
void setup()
{
  ...
  OCR1A = 49999;  // = 16000000 / (256 * 1.25) - 1 (deve ser < 65536)
  // ligando CTC mode
  TCCR1B |= (1 << WGM12);
...
{
  //maximo de endereco 2047, endereco 2048 = endereco 0. Valor maximo 255, maior que isso e os bits mais significativos sao cortados, ex: 256 = 0
  EEPROM.write(endereco, dado); // Comando que escreve um dado em determinado endereco na EEPROM
}
...
{
  EEPROM.write(endereco, dado1);  // Comando que escreve um dado1 em determinado endereco na EEPROM
  EEPROM.write(endereco + 1, dado2);  // Comando que escreve um dado2 em determinado endereco+1 na EEPROM
}

//FUNCAO LERN: usamos uma String separada pelo caracter "," para armazenar os valores lidos. Cada numero entre virgulas corresponde um byte lido a partir do endereco dado ate o endereco + n
String lerN_EEPROM(unsigned int enderecoInicial, unsigned int n)
{
  String saida;

  for (int i = 0; i < n; i++)
  {
    saida = saida + "," + EEPROM.read(enderecoInicial + i);
  }
...
void funcaoCaso1()
{
  ...
  memoriaDeletada = enderecoEscrita;
  lcd.clear();
  ...
  lcd.print("DELETADAS");
  enderecoEscrita = 0;//zeramos nosso endereco de escrita de dados na EEPROM, novos dados serao escritos em cima dos dados antigos 
  delay(3000);
  ultimosDois = "-*";
}
...
void funcaoCaso5()
{
  ...
  if (!cancelaFunc)
  {
    ...
    else
    {
      ...
      //lendo medidas
      for (int i = 0; i < tamanhoN * 2; i++)
      {
        if (i % 2)
        {
          Serial.println((int)(((unsigned) EEPROM.read(enderecoEscrita *2 - i) << 8) | EEPROM.read(enderecoEscrita *2 - 1 - i)));//lemos de dois em dois bytes (limite de 1 byte 256, precisamos de dois little endian) movimentando os bits de forma a reconstruir o int original
        }
      }
